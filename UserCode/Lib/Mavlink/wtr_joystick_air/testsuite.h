/** @file
 *    @brief MAVLink comm protocol testsuite generated from wtr_joystick_air.xml
 *    @see https://mavlink.io/en/
 */
#pragma once
#ifndef WTR_JOYSTICK_AIR_TESTSUITE_H
#define WTR_JOYSTICK_AIR_TESTSUITE_H
...
static void mavlink_test_wtr_joystick_air(uint8_t, uint8_t, mavlink_message_t *last_msg);

static void mavlink_test_all(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{

    mavlink_test_wtr_joystick_air(system_id, component_id, last_msg);
}
#endif
...
static void mavlink_test_joystick_air_dashboard(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_JOYSTICK_AIR_Dashboard >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_joystick_air_dashboard_t packet_in = {
        17.0,17,"FGHIJKLMNOPQRSTUVWX"
    };
    mavlink_joystick_air_dashboard_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_JOYSTICK_AIR_Dashboard_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_JOYSTICK_AIR_Dashboard_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_joystick_air_dashboard_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_joystick_air_dashboard_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_joystick_air_dashboard_pack(system_id, component_id, &msg , packet1.id , packet1.title , packet1.value );
    mavlink_msg_joystick_air_dashboard_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_joystick_air_dashboard_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.id , packet1.title , packet1.value );
    mavlink_msg_joystick_air_dashboard_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_joystick_air_dashboard_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_joystick_air_dashboard_send(MAVLINK_COMM_1 , packet1.id , packet1.title , packet1.value );
    mavlink_msg_joystick_air_dashboard_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

#ifdef MAVLINK_HAVE_GET_MESSAGE_INFO
    MAVLINK_ASSERT(mavlink_get_message_info_by_name("JOYSTICK_AIR_Dashboard") != NULL);
    MAVLINK_ASSERT(mavlink_get_message_info_by_id(MAVLINK_MSG_ID_JOYSTICK_AIR_Dashboard) != NULL);
#endif
}

static void mavlink_test_wtr_joystick_air(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    mavlink_test_joystick_air(system_id, component_id, last_msg);
    mavlink_test_joystick_air_led(system_id, component_id, last_msg);
    mavlink_test_joystick_air_dashboard(system_id, component_id, last_msg);
}

#ifdef __cplusplus
}
#endif // __cplusplus
#endif // WTR_JOYSTICK_AIR_TESTSUITE_H
